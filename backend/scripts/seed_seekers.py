...
async def seed_seekers_data():
    ...
    for i in range(0, len(expanded_seekers), 10):
        ...
        for s in chunk:
            ...
            u = await _seed_auth_user(email=f"{random.randint(100,999)}_{s['email']}", name=mutated_name, role=UserRole.SEEKER.value)
            edu_objs = [Education(
            ...
            seeker = SeekerProfile(
                ...
                resume_text=s["resume"], salary_expectation_min=s["sal"][0] + random.randint(-500000, 1000000),
